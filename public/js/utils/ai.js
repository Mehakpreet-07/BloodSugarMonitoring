...
export function makeAiAdvice(readings = [], thresholds) {
  ...
  const withCat = readings.map(r => {
      ...
      // ⭐ FIX: Recalculate category if not present or outdated
      if (!cat) {
        const val = r.valueMgdl || r.valueMgPerdL;
        if (val < abnormalMaxMg) cat = 'AbnormalLow';
        else if (val >= abnormalHighMinMg) cat = 'AbnormalHigh';
        else if (val <= normalMax) cat = 'Normal';
        else cat = 'Borderline';
      }
  ...
  const abnormalHighs = withCat.filter(r => r.cat === 'AbnormalHigh');
  const abnormalLows  = withCat.filter(r => r.cat === 'AbnormalLow');
  const triggerCounts = {};
  ...
  // ⭐ FIX: Only show triggers with 2+ occurrences
  Object.entries(triggerCounts).forEach(([trigger, count]) => { 
    if (count >= 2) {
      advice.push({ level: 'warning', text: `Pattern detected: High glucose frequently occurs after '${trigger}'.` }); 
    }
  });

  if (abnormalHighs.length >= 3) {
    advice.push({ level: 'warning', text: 'Multiple high readings detected recently. Review your meal plan.' });
  }
  
  if (abnormalLows.length > 0) {
    advice.push({ level: 'caution', text: 'Low glucose event detected. Keep fast-acting carbs nearby.' });
  }
  
  // ⭐ NEW: Positive feedback
  const normalCount = withCat.filter(r => r.cat === 'Normal').length;
  if (normalCount > (readings.length * 0.7) && readings.length >= 5) {
    advice.push({ level: 'positive', text: 'Excellent! Over 70% of your readings are normal. Keep it up!' });
  }
  
  if (advice.length === 0 && readings.length > 0) {
    advice.push({ level: 'positive', text: 'No negative patterns detected. Great job!' });
  } else if (readings.length === 0) {
    advice.push({ level: 'tip', text: 'Start logging food and events to unlock AI insights.' });
  }
